...
async def test_blocking_input_guardrail_delays_tool_execution():
    ...
    blocking_guardrail = InputGuardrail(
        guardrail_function=slow_blocking_guardrail,
    ...
    model.add_multiple_turn_outputs([
        [get_function_tool_call("test_tool", "{}")],
        [get_text_message("completed")]
    ])
...
async def test_mixed_blocking_and_non_blocking_guardrails():
    ...
    blocking_gr = InputGuardrail(
        guardrail_function=blocking_guardrail,
        block_tool_calls=True
    )
    non_blocking_gr = InputGuardrail(
        guardrail_function=non_blocking_guardrail,
        block_tool_calls=False
    )
