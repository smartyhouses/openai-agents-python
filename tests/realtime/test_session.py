...
class MockRealtimeModel(RealtimeModel):
    ...
    async def send_message(self, message, other_event_data):
...
class TestEventHandling:
    ...
    async def test_item_updated_event_updates_existing_item(self, mock_model, mock_agent):
        ...
        assert len(session._history) == 1
        assert session._history[0].content[0].text == "Updated"
...
class TestHistoryManagement:
    ...
    def test_merge_transcript_into_existing_audio_message(self):
        ...
        new_history = RealtimeSession._get_new_history(old_history, transcription_event)

        # Verify the transcript was merged
        assert len(new_history) == 1
        updated_item = new_history[0]
        ...
        assert updated_item.content[0].text == "Before audio"
        assert updated_item.content[1].transcript == "Hello world"
        assert updated_item.content[1].audio == "audio_data"  # Should preserve audio data
        assert updated_item.content[2].text == "After audio"
    ...
    def test_merge_transcript_preserves_other_items(self):
        ...
        assert new_history[1].content[0].transcript == "Transcribed audio"
        assert new_history[1].status == "completed"
    ...
    def test_merge_transcript_only_affects_matching_audio_content(self):
        ...
        assert updated_item.content[0].text == "Text content"
        assert updated_item.content[3].text == "More text"

        # All audio content should have the new transcript (current implementation overwrites all)
        assert updated_item.content[1].transcript == "New transcript"
        assert (
            updated_item.content[2].transcript == "New transcript"
    ...
    def test_update_existing_item_by_id(self):
        ...
        new_history = RealtimeSession._get_new_history(old_history, updated_item)
        ...
        assert new_history[0].content[0].text == "Updated"
    ...
    def test_update_existing_item_preserves_order(self):
        ...
        new_history = RealtimeSession._get_new_history(old_history, updated_item2)
        ...
        assert new_history[1].content[0].text == "Updated Second"

        # Other items should be unchanged
        assert new_history[0].content[0].text == "First"
        assert new_history[2].content[0].text == "Third"
    ...
    def test_insert_new_item_after_previous_item(self):
        ...
        assert new_history[1].content[0].text == "Second"
    ...
    def test_complex_insertion_scenario(self):
        ...
        assert len(history) == 4
        assert [item.item_id for item in history] == ["A", "B", "D", "C"]
        assert history[1].content[0].text == "Updated B"
